...
# You can set CONSUL_ADVT_INTERFACE to the name of the interface you'd like to
# advertise to the cluster and this will look up the IP and pass the proper -advertise= option along
# to Consul.
CONSUL_ADVT=
if [ -n "$CONSUL_ADVT_INTERFACE" ]; then
  CONSUL_ADVT_ADDRESS=$(ip -o -4 addr list $CONSUL_ADVT_INTERFACE | head -n1 | awk '{print $4}' | cut -d/ -f1)
  if [ -z "$CONSUL_ADVT_ADDRESS" ]; then
    echo "Could not find IP for interface '$CONSUL_ADVT_INTERFACE', exiting"
    exit 1
  fi

  CONSUL_ADVT="-advertise=$CONSUL_ADVT_ADDRESS"
  echo "==> Found address '$CONSUL_ADVT_ADDRESS' for interface '$CONSUL_ADVT_INTERFACE', setting bind option..."
fi
